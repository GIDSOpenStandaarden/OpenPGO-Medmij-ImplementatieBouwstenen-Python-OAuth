...
The medmij_oauth package assists in implementing an oauth server/client application conform the medmij oauth flow (:ref:`described here <the-medmij-oauth-flow>`). The module consists of 3 main submodules i.e. :ref:`medmij_oauth.server <medmij_oauth.server>`, :ref:`medmij_oauth.client <medmij_oauth.client>` and :ref:`medmij_oauth.exceptions <medmij_oauth.exceptions>`.
...
.. _medmij_oauth.server:

Server
======

API Reference: :mod:`medmij_oauth.server <medmij_oauth.server>`
...
The methods that you need to implement can be found on the :class:`DataStore ABC <medmij_oauth.server.DataStore>`.
...
:ref:`More info <server.oauthsession>`
...
.. _medmij_oauth.client:

Client
======

API Reference: :mod:`medmij_oauth.client <medmij_oauth.client>`
...
Your implementation of the DataStore class handles instantiation, persisting and lookups of OAuthSessions.
The methods that you need to implement can be found on the :class:`DataStore ABC <medmij_oauth.client.Datastore>`.
...
:ref:`More info <client.oauthsession>`
...
Used by the client to make a :meth:`exchange_authorization_code <medmij_oauth.client.Client.exchange_authorization_code>` request to the oauth server.
...
.. _medmij_oauth.exceptions:

Exceptions
==========

API Reference: :mod:`medmij_oauth.exceptions <medmij_oauth.exceptions>`
...
The different possible errors are contained in the :class:`ERRORS <medmij_oauth.exceptions.ERRORS>` enum. Further optional arguments that the OAuthException's constructor takes are, *error_descripion*, *redirect* and *redirect_url*.
...
.. _the-medmij-oauth-flow:

The MedMij OAuth flow
=====================

In the API references you find links to this flow, that means that those functions assist in implementing this step of the oauth flow. (e.g. :meth:`Server.create_oauth_session <medmij_oauth.server.Server.create_oauth_session>`)
...
License
=======
This project is licensed under the AGPL-3.0 License - see the `LICENSE <https://github.com/GidsOpenStandaarden/OpenPGO-Medmij-ImplementatieBouwstenen-Python-OAuth/blob/master/LICENSE>`__ file for details